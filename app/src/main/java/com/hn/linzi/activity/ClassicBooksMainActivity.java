...
public class ClassicBooksMainActivity extends BaseActivity {
			...
			public boolean onEditorAction(TextView v, int actionId,
				...
				if (actionId == EditorInfo.IME_ACTION_SEARCH) {
					if (edit.getText().toString().equals("")) {
						Toast.makeText(thiscontext, "搜索不能为空",
						...
						try {
							url = "http://data.iego.net:85/m/booklist1Json.aspx?"
									...
									+ "&d="
									+ sp.getString("KEY", "")
									+ "&lib=classic&size=18&page=1&kwd="
									+ URLEncoder.encode(edit.getText()
											.toString(), "UTF-8");
							// n=201010412001&d=5744f175c7489fd2f3ccee928d6d7cc8&lib=classic&size=18&page=1&kwd=" + URLEncoder.encode(edit.getText().toString(), "UTF-8");
						...
						intent.setClass(thiscontext,
								ClassicBooksListActivity.class);
	...
	private void getClassic() {
		new Thread() {
			...
			@Override
			public void run() {
				String content = "http://data.iego.net:85/m/bookClassJson.aspx?lib=classic";
				URL url;
				try {
					url = new URL(content);
					...
					if (conn.getResponseCode() == 200) {
						...
						message.what = ClassicBooksMainActivity.REFRESH;
	...
	public Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
			case ClassicBooksMainActivity.REFRESH:
				...
				gridView.setOnItemClickListener(new OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> arg0, View arg1,
						...
						if (item.get("ItemName").toString().equals("全部")) {
							String url = "http://data.iego.net:85/m/booklist1Json.aspx?"
									+ "&n="
									+ sp.getString("UserName", "")
									+ "&d="
									+ sp.getString("KEY", "")
									+ "&lib=classic&size=18&page=1";
							// n=201010412001&d=5744f175c7489fd2f3ccee928d6d7cc8&lib=classic&size=18&page=1";
							Bundle bundle = new Bundle();
							Intent intent = new Intent();
							bundle.putString("url", url);
							...
							try {
								url = "http://data.iego.net:85/m/booklist1Json.aspx?"
										...
										+ "&d="
										+ sp.getString("KEY", "")
										+ "&lib=classic&size=18&page=1&class1="
										+ URLEncoder.encode(item
												.get("ItemName").toString(),
												"UTF-8");
								// n=201010412001&d=5744f175c7489fd2f3ccee928d6d7cc8&lib=classic&size=18&page=1&class1="  + URLEncoder.encode( item.get("ItemName").toString(), "UTF-8");
							} catch (UnsupportedEncodingException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
					...
					if (button.getText().toString().equals("全部")) {
						...
						try {
							url = "http://data.iego.net:85/m/booklist1Json.aspx?"
									...
									+ sp.getString("KEY", "")
									+ "&lib=classic&size=18&page=1&class1="
									+ URLEncoder.encode(button.getText()
											.toString(), "UTF-8");
						} catch (UnsupportedEncodingException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
