...
public class ClassicBooksContentActivity extends BaseActivity {
		...
		StringRequest request = new StringRequest(url, new Listener<String>() {
			...
			public void onResponse(String response) {
				...
				try {
					...
					message.what = ClassicBooksContentActivity.REFRESH;
	...
	public Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
			case ClassicBooksContentActivity.REFRESH:
...
//							String path = DL.getPath() + "/" + sp.getString("UserName", "") + "/";
							if (data.get(8).equals("epub")) {
								...
								path = path + sp.getString("UserName", "")+ "_" + data.get(1)
										+ ".epub";
