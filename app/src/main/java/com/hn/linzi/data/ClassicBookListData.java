...
public class ClassicBookListData {
	...
	private ClassicBooksListActivity classicBooksListActivity;
	private String contentURL;
	private String type;
	private String word;
	private Context context;
	
	public ClassicBookListData(ClassicBooksListActivity classicBooksListActivity, String url, String type, String word, Context context) {
		this.classicBooksListActivity = classicBooksListActivity;
		this.contentURL = url;
		this.type = type;
		this.word = word;
...
//				String contentURL = "http://data.iego.net:88/lesson/lessonListJson.aspx?imei=123456-78-901234-5&n=201010412001&d=5744f175c7489fd2f3ccee928d6d7cc8&code=88888888&size=18&page=1";
				...
				try {
					...
					if (conn.getResponseCode() == 200) {
						...
						parseJson.parseClassicBookList(inStream, IMG_DESCRIPTIONS); 
					}
				} catch (IOException e) {
...
//				queue.add(request);
		        ...
		        message.what = ClassicBooksListActivity.REFRESH;
		        classicBooksListActivity.handler.sendMessage(message);
				super.run();
			}
		...
		new Thread(){
			...
			public void run() {
				...
				if(type.equals("sousuo")){
					try {
						moreData = "http://data.iego.net:85/m/booklist1Json.aspx?n=" + sp.getString("UserName", "") +
								"&d=" +sp.getString("KEY","" ) + "&lib=classic&size=18&page=" + i + "&kwd=" + URLEncoder.encode(word, "UTF-8");
					} catch (UnsupportedEncodingException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					};
				...
				if(type.equals("quanbu")){
					moreData = "http://data.iego.net:85/m/booklist1Json.aspx?n=" + sp.getString("UserName", "") +
							"&d=" +sp.getString("KEY","" ) + "&lib=classic&size=18&page=" + i;
				}
				if(type.equals("fenlei")){
					try {
						moreData = "http://data.iego.net:85/m/booklist1Json.aspx?n=" + sp.getString("UserName", "") +
								"&d=" +sp.getString("KEY","" ) + "&lib=classic&size=18&page=" + i + "&class1=" + URLEncoder.encode(word, "UTF-8");
					} catch (UnsupportedEncodingException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					};
...
//				String contentURL = "http://data.iego.net:88/lesson/lessonListJson.aspx?imei=123456-78-901234-5&n=201010412001&d=5744f175c7489fd2f3ccee928d6d7cc8&code=88888888&size=18&page=" + i;
				...
				Message message = new Message();
		        message.what = ClassicBooksListActivity.ADAPTER;
		        classicBooksListActivity.handler.sendMessage(message);
				super.run();
			}
